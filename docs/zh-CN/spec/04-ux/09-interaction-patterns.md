...
启动器以空查询打开，按渲染器本地的设备历史，以最近使用顺序展示已启用且就绪的
面板插件，因此上一次打开的插件始终只差一个 Enter。输入时仍然优先按搜索相关性
排序；最近使用只用于在同等相关的匹配之间打破平局。
...
- 在 Windows 和 Linux 上，显式的应用最小化意味着**原生任务栏最小化**：
  渲染器的窗口控制按钮与原生菜单里的最小化操作，走的是正常的操作系统过渡。
  macOS 的交通灯最小化按钮以及 macOS 窗口 → 最小化角色，仍然是**隐藏到托盘**。
- 在 Windows 上，点击当前聚焦且可见的主窗口的任务栏按钮，意味着**原生最小化**。
  该窗口仍由它的任务栏条目代表；下一次点击会恢复并聚焦它。当窗口只是被遮挡时，
  点击任务栏会把它带到最前，而不会把它隐藏到托盘。
- 隐藏到托盘——包括 Windows/Linux 上配置为 `tray` 的关闭行为——会把主窗口从
  taskbar/dock 的窗口列表中移除，而 Electron 进程与后台工作仍然存活。它不会
  持久化一个最小化的几何尺寸，也不会销毁 host/sidecar。
...
- **重命名** —— 会话行菜单与"项目归档"任务行打开的是同一个模态编辑器。保存时
  会裁剪标题并持久化 1–80 个 Unicode 码点；空值不可提交。标题只是元数据，
  因此该任务的转录、活动排序、项目绑定与空会话状态都不受影响。Escape、取消，
  或点击遮罩层都会关闭编辑器。
...
- 首次打开的会话在其最新回合处落定。重新访问的面板回到用户离开的
  ...
  不再重置手动滚动状态（ADR 0137）。
- 选择项目范围的对话会激活其项目作为
  同一次由 store 拥有的选择事务的一部分。选择临时对话则清空可见的工作区。
  项目范围的新建会话操作，把它们的目标路径传给同一次 store 事务；侧边栏与
  项目索引的处理器，不会在创建或选择会话之前再执行一次项目导航。
...
- 会话运行期间，发送按钮与中止按钮一同保持可用。被接受的提示会清空输入框，
  并追加到该会话在渲染器本地的 FIFO 队列；切换会话绝不会移动或清空另一个
  会话的队列。
- 队列渲染在输入框上方。每一行都有一个可独立通过键盘到达的"移除"操作和一个
  "立即发送"操作。
- "立即发送"把该行移到队首，并请求新的 `agent/stop` 通道。当前的助手响应与
  已完成的这批工具正常收尾；在 `agent_end` 之后，被提升的那一行先于其余行，
  经由正常的 `agent/prompt` 流程派发。空闲状态下的"立即发送"会立即派发。
- 中止仍然是立即生效的，并且绝不清空队列。排队的提示在应用重启时会被有意
  丢弃，因为该队列并不持久。
...
工作面板与应用窗口的尺寸调整都已在 MVP 中实现：

- 10px 的内侧左边缘分隔条锚定到按下时的位置与起始面板宽度，随后跟随指针
  增量移动而不跳变。向左移动会让面板变宽；向右移动则把空间还给 MainChat。
- 内侧分隔条的目标宽度被钳制在渲染器拥有的面板范围 `244px–720px` 内；指针
  移动按帧合并，释放时提交首选宽度。Escape、指针取消以及捕获丢失，都会恢复
  按下时的面板宽度。
- 打开与关闭会同时对 dock 的 `width` 与 `flex-basis` 做动画，并伴随有界的
  opacity/transform 反馈，因此 MainChat 在既有的客户区内持续回流，而不是在
  第一个动画帧之前就改变宽度。
- 没有任何面板操作会请求一个正数的原生预留宽度。首选面板宽度是渲染器本地的，
  原生窗口边缘只调整固定的应用窗口。后台会话的产物绝不会更新可见面板或窗口
  的几何尺寸。
- 原生 Browser 视图仍然跟随渲染器测得的面板矩形；由于它无法参与渲染器的 CSS
  动画，会在折叠动作之前先被分离。原生边界的恢复与持久化，继续适用于普通的
  窗口调整/移动手势，不掺入面板专属的增量。
...
- 悬停侧边栏主体时边缘保持透明。悬停手柄只显示一个居中的紧凑标记；聚焦与
  正在拖动时该标记保持可见，但不会画出整条通高轨道，也不会让布局位移。
- 折叠侧边栏会隐藏手柄，但不会丢弃首选的展开宽度；重新展开时恢复该宽度。
...
- 当图片引用处于激活状态时，Composer 显示一行紧凑的实时状态。对于 pi-ai
  `input` 包含 `image` 的模型，它标明走视觉传输；对于未知/非视觉模型，它标明
  回退到文件路径。该状态是信息性的、对键盘友好的，并且绝不单靠颜色来表达。
...
- 第一次**向上滚动手势**（滚轮 / 触控板 / 触摸 / 滚动条 / 键盘）会暂停自动
  滚动并显示"↓ 滚动到底部"按钮；排队的流式或尺寸变化跟随工作，不得逆转这次
  移动
- 程序化的跟随滚动与布局驱动的钳制，绝不释放跟随：一个前面没有用户输入的滚动
  事件（例如发送后输入框收起时的 scrollTop 钳制，或某个指示行卸载）被当作
  布局噪声重新取基线，而不是被误认为用户在向上滚动
...
- 悬停或聚焦某个会话行时，会在与项目路径工具提示相同的 500ms 延迟之后，
  显示一张多行悬浮卡；该卡片绝不会锚定到一个已被拆除的行上。
- 卡片自上而下按此顺序呈现该行的元数据：标题、标签芯片、**工作区**、分支
  （当项目提供分支时），以及 **更新于 {{when}}**。临时/草稿会话显示本地化的
  "Temporary" / "临时对话"占位文本，而不是工作区名称。
- 卡片通过挂在 `document.body` 上的 portal 渲染，宽度绝不超过 320px，绝不
  让下方的行产生横向滚动，并保持非交互，因此该行仍然接收指针事件。
- 会话行不设置原生的 `title` 属性。悬浮卡是唯一呈现完整标题的界面，因此浏览器
  的工具提示绝不会叠加在卡片上。
- 卡片在指针离开、焦点失去、滚动（任意滚动容器）、窗口尺寸变化，以及右键菜单
  打开的那一刻取消。
...
- 手动向上移动会先取消排队的固定跟随帧
  ...
  显式的启动/跳转到最新操作会重新固定它。
- 释放跟随的检测，以最近一次用户滚动输入为前提。来自某次跟随 `scrollTo`、
  其位置随后被布局变化（输入框高度、指示行）钳制过的原生滚动事件，是事后才
  到达的，看上去像一次向上手势；因为它们前面没有输入，所以被忽略，跟随模式
  得以保留。
