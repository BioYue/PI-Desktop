...
市场安装会在下载之前立即刷新目录，因此 URL 与校验和是从同一份当前的元数据
快照中解析出来的。在线安装不信任较新的 UI 缓存；如果刷新失败，可以使用上一份
有效目录以支持离线操作，而校验和要求依然适用。
...
桌面端的实现是按已安装插件逐个选择加入的。打开扩展页面会静默检查远程目录；
显式的"检查更新"操作则总是抓取一份新的目录，离线时回退到上一份有效缓存。
"应用自动更新"只升级那些启用了自动更新、且不引入新权限的插件，并且在安装
之前仍然校验软件包的校验和。会新增权限的更新仍然走人工审核流程。
