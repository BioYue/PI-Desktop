...
版本条目在目录里是无序输入。主机解析语义化版本，并选出最高的合法版本作为
`latestVersion`；它不会假定数组的第一项就是最新的。详情响应按最新优先排序
版本，而不指定版本的安装指向的正是同一个最新版本。非法的版本字符串优先级低于
合法的语义化版本。缺少 `shasum` 或 `url` 的版本在发现中仍然可见，但在发布者
补全其包元数据之前不可安装。
...
主机在市场下载之前会立即刷新目录，因此包 URL 与校验和来自同一份当前的目录
快照。这可以防止短命的 UI/目录缓存，与一个可变发布 URL 上更新的包配成一对。
如果刷新不可用，主机可以使用上一份有效目录进行离线安装，但它仍然会拿下载到的
字节去校验那份目录里的校验和。
...
1. 扩展页面打开时或按计划执行 `checkUpdates`
2. 把已安装版本与最新版本比较
3. UI 显示可用更新的列表
4. 用户确认后下载并升级

策略：
- 扩展页面打开时，只用上一份有效的本地目录静默检查已安装项的更新元数据；
  这次仅用缓存的检查，不得让主机 RPC 状态锁卡在一次市场网络超时后面。
- 一次显式检查会刷新远程目录，离线时回退到上一份有效缓存。
- 可选的自动更新仍然是按插件的，并且绝不静默接受一项新权限。
...
扩展列表页把它的概览文案保持紧凑：页面标题与标签页标签标明这是什么界面，而
分节标题与空状态使用直接的标签和操作，不写解释性段落。解释留在真正有助于做
决定的地方——权限审核、详情表、编辑器和错误状态。
...
一次显式的更新检查会执行一次新的远程目录抓取，离线时回退到上一份有效的本地
目录。打开扩展页面还会执行一次仅用缓存的静默更新检查，好让已安装的行能够刷新，
而不必让市场界面等待一次远程请求。市场标题栏的刷新操作仍然是显式的远程刷新
路径。
